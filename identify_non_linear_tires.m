...
% 3. Extract the "Golden" Data (Smoothed by your EM/RTS step)
% Note: This assumes you ran the EM script and have 'x_smooth' in workspace.
% If not, this block relies on variables existing from previous runs.
% (Assuming x_smooth is already calculated by your EM_EKF_tuner)

% Test estimated angular acceleration vs ground truth
...
for t = 1:N
    ...
    ddy_body = Yv * vy_t + Yr * r_t + (Caf_t * T * delta_t / m) + (xi_y_t / m);
    
    % Inertial Frame Conversion (Matches IMU ground truth):
    % ay = dvy_body/dt + vx*r
    % Note: Since Yr contains -vx, adding +vx*r cancels it out, leaving just forces.
    ddy_reconstructed(t) = ddy_body + (vx_t * r_t);

    % --- B. Reconstruct Variance ---
    P_t = r_smooth(:, :, t); % Assuming r_smooth is the smoothed covariance
...
figure; 
plot(time_vector, ddy_reconstructed, 'b'); hold on;
plot(time_vector, out.gt_ay.Data, 'r--');
...
% Bounds: Allow residuals (9-11) to be non-zero!
...
options = optimoptions('fmincon', 'Display', 'off', 'Algorithm', 'sqp', ...
                       'MaxIterations', 2000, 'StepTolerance', 1e-8);
...
% %     delta_t = logged_u(t, 2);
% %     Caf_t = veh_param.Caf * out.FzF.Data(t); Car_t = veh_param.Car * out.FzR.Data(t);
% %     % Caf_t = out.Caf.Data(t); Car_t = out.Car.Data(t);
% % 
% %     % Linear Model Terms
% %     vx_safe = max(vx_t, 1.0);
...
% %     Nr = -(Caf_t*lf^2 + Car_t*lr^2) / (Izz * vx_safe);
% %     Yv = -(Caf_t + Car_t) / (m * vx_safe);
% %     Yr = -vx - (Caf_t*lf - Car_t*lr) / (m * vx_safe);
% %     B_lin_psi = (Caf_t*lf) / Izz;
% % 
% % 
% %     % A. Reconstruct Target (Mean)
% %     ddy_lin = Yv * vy_t + Yr * r_t + (Caf_t * T * delta_t / m);
...
% loss_handle = @(p) loss_function(p, states_smooth, inputs_log, Fz_data, consts, ddy_recon);
% % loss_handle = @(p) loss_function_weighted(p, states_smooth, inputs_log, Fz_data, consts, ddy_recon, y_target_variance, initial_guess);
% 
% % --- 5. Run MultiStart ---
% fprintf('Step 3: Running MultiStart...\n');
% options = optimoptions('fmincon', 'Display', 'off', 'Algorithm', 'sqp', ...
%                        'MaxIterations', 10000, 'StepTolerance', 1e-8);
% 
% problem = createOptimProblem('fmincon', ...
%     'objective',   loss_handle, ...
%     'x0',          initial_guess, ...
...
% [theta_optimal, final_loss] = run(ms, problem, 200); 
% 
% % --- 6. Results ---
% fprintf('\nOptimization Finished.\n');
% fprintf('Final Weighted Loss: %f\n', final_loss);
